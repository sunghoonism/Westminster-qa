...
class QALargePage extends StatefulWidget {
...
class _QALargePageState extends State<QALargePage> {
  ...
  Future<void> _fetchQAList() async {
    final list = await widget.database.query('large');
    setState(() {
      qaList = list;
      _filterList();
    });
  ...
  void _onScroll() {
    // 200ms debounce로 저장 빈도 최적화
    _debounce?.cancel();
    _debounce = Timer(const Duration(milliseconds: 200), () async {
      final prefs = await SharedPreferences.getInstance();
      prefs.setDouble('large_scroll_offset', _scrollController.offset);
    });
  }

  Future<void> _restoreScrollOffset() async {
    final prefs = await SharedPreferences.getInstance();
    final offset = prefs.getDouble('large_scroll_offset') ?? 0.0;
    // 위젯 트리 빌드 후 이동
